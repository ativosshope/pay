...
try {
    ...
    JsonStorage::log('ghostpay_webhook_raw', [
    ...
    if (!$internalOrderId && !empty($transaction['items'][0]['title'])) {
        if (preg_match('/ORD-\d+/', (string)$transaction['items'][0]['title'], $matches)) {
            $internalOrderId = $matches[0];
        }
    }

    $rawStatus = strtolower((string)($transaction['status'] ?? $payload['status'] ?? ''));
    $status = normalizePaymentStatus('ghostpay', $rawStatus);

    JsonStorage::log('ghostpay_status_processing', [
    ...
    if (!$order) {
        $ghostPayId = $transaction['id'] ?? $payload['id'] ?? null;
        $order = $ghostPayId ? JsonStorage::findBy('transacoes', 'gateway_id', (string)$ghostPayId) : null;
        if ($order) {
            JsonStorage::log('ghostpay_found_by_gateway_id', [
                'gateway_id' => (string)$ghostPayId,
    ...
    if (!$order) {
        JsonStorage::log('ghostpay_order_not_found', [
            'internal_order_id' => $internalOrderId,
    ...
    $result = compensatePayment($order, 'ghostpay', $rawStatus, $paidAt);

    JsonStorage::log('ghostpay_status_updated', [
        'order_id' => $order['order_id'],
    ...
    try {
        JsonStorage::log('ghostpay_webhook_error', ['error' => $e->getMessage()]);
    } catch (Throwable $logError) {
        error_log('ghostpay webhook log failed: ' . $logError->getMessage());
    }
