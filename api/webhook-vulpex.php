...
try {
    ...
    JsonStorage::log('vulpex_webhook_raw', [
        'event' => $payload['event'] ?? 'unknown',
        'transaction_id' => $payload['transaction']['id'] ?? $payload['id'] ?? 'unknown',
    ...
    $status = normalizePaymentStatus('vulpex', $rawStatus);

    JsonStorage::log('vulpex_status_processing', [
    ...
    if (!$order) {
        $vulpexId = $transaction['id'] ?? $transaction['transaction_id'] ?? $payload['id'] ?? null;
        $order = $vulpexId ? JsonStorage::findBy('transacoes', 'gateway_id', (string)$vulpexId) : null;
        if ($order) {
            JsonStorage::log('vulpex_found_by_gateway_id', [
                'gateway_id' => (string)$vulpexId,
    ...
    if (!$order) {
        JsonStorage::log('vulpex_order_not_found', [
            'internal_order_id' => $internalOrderId,
    ...
    $result = compensatePayment($order, 'vulpex', $rawStatus, $paidAt);

    JsonStorage::log('vulpex_status_updated', [
        'order_id' => $order['order_id'],
    ...
    try {
        JsonStorage::log('vulpex_webhook_error', ['error' => $e->getMessage()]);
    } catch (Throwable $logError) {
        error_log('vulpex webhook log failed: ' . $logError->getMessage());
    }
